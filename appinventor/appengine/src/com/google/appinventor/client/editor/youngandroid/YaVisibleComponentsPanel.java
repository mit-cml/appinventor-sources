...
public class YaVisibleComponentsPanel extends SimpleVisibleComponentsPanel<MockForm> {
  ...
  public void show(MockForm root) {
    this.root = root;
    HiddenComponentsManager manager = HiddenComponentsManager.getInstance();
    manager.setCurrentForm(root);
    Boolean state = Ode.getCurrentProjectEditor().getScreenCheckboxState(root.getTitle());
    boolean effectiveState = (state != null) ? state : false;
    LOG.info("Setting checkbox state for " + root.getTitle() + " to " + effectiveState);

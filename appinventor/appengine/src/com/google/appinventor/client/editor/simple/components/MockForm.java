 ...
 * corresponding to 'normal' and 'large' buckets
 * (http://developer.android.com/guide/practices/screens_support.html). Normal
 * size is a 1:1 with pixels on a device with dpi:160. We use that as the
 * baseline for the browser too. All UI elements should be scaled to DP for
 * buckets other than 'normal'.
 */
public final class MockForm extends MockContainer {
  ...
  public void changePreviewSize(int width, int height) {
    // It will definitely modified in the future.
  ...
  protected void addWidthHeightProperties() {
    addProperty(PROPERTY_NAME_WIDTH, "" + PORTRAIT_WIDTH, null, new YoungAndroidLengthPropertyEditor());
    addProperty(PROPERTY_NAME_HEIGHT, "" + LENGTH_PREFERRED, null, new YoungAndroidLengthPropertyEditor());
  }
  ...
  private void setIconProperty(String icon) {
    ...
    if (editor.isScreen1()) {
      editor.getProjectEditor().changeProjectSettingsProperty(SettingsConstants.PROJECT_YOUNG_ANDROID_SETTINGS,
   ...
   *
   * Each components onPropertyChange listener calls us. This is reasonable during
   * interactive editing because we have to make sure the screen reflects what the
   * user is doing. However during project load we will be called many times, when
   * really we should only be called after the project's UI is really finished
   * loading.
   *
   * We could add a bunch of complicated code to inhibit refreshes until we know
   * the project's UI is loaded and stable. However that is a change that will be
   * spread over several modules, making it hard to understand what is going on.
   *
   * Instead, I am opting to keep this change self contained within this module.
   * The idea is to see how quickly we are being called. If we receive a call
   * which is close in time (within seconds) of a previous call, we set a timer to
   * fire in the reasonable future (say 2 seconds). While this timer is counting
   * down, we ignore any other calls to refresh. Whatever refreshing they would do
   * will be handled by the call done when the timer fires. This approach does not
   * reduce the number of calls to refresh during project loading to 1. But it
   * significantly reduces the number of calls and gets us out of the exponential
   * explosion in time and memory that we see with projects with hundreds of
   * design elements (yes, people do that, and I have seen at least one project
   * that was this big and reasonable!). -Jeff Schiller (jis@mit.edu).
   *
   */
   ...
   * refreshes which bypass throttling.
   *
   */
  ...
  public final void doRefresh() {
    ...
    rootPanel.setPixelSize(formLayoutInfo.width, Math.max(formLayoutInfo.height, usableScreenHeight));
  ...
  /*
   * Collects the LayoutInfo of the given component and, recursively, all of its
   * children.
   *
   * If a component's width/height hint is automatic, the corresponding
   * LayoutInfo's width/height will be set to the calculated width/height. If a
   * component's width/height hint is fill parent, the corresponding LayoutInfo's
   * width/height may be set to fill parent. This will be resolved when
   * layoutChildren is called.
   */
  private static void collectLayoutInfos(Map<MockComponent, LayoutInfo> layoutInfoMap, MockComponent component) {
   ...
   * <p>
   * There will always be exactly one component selected in a form at any given
   * time.
   */
  ...
  public EditableProperties getProperties() {
    ...
    if (!editor.isScreen1()) {
      properties.changePropertyValue(SettingsConstants.YOUNG_ANDROID_SETTINGS_SIZING,
          editor.getProjectEditor().getProjectSettingsProperty(SettingsConstants.PROJECT_YOUNG_ANDROID_SETTINGS,

...
public final class YaFormEditor extends DesignerEditor<YoungAndroidFormNode, MockForm,
  ...
  protected void onFileLoaded(String content) {
    ...
    LOG.info("websocket provider ready");
    JSONObject propertiesObject = YoungAndroidSourceAnalyzer.parseSourceFile(
        content, JSON_PARSER);
    try {
  ...
  private native void initWebsocketProvider(String projectId) /*-{
    ...
    doc.getMap('components').observeDeep(function(events) {
      events.forEach(function(event) {
        if (event.transaction.local) return;  // ignore my own writes to avoid duplicate updates
        ...
        if (event.path.length === 0) {
          event.changes.keys.forEach(function(change, uuid) {
            if (change.action === 'add') {
              ...
              var componentJson = JSON.stringify(event.target.get(uuid).toJSON());
        ...
        // handles nested changes
          ...
          var uuid = event.path[0];
          event.changes.keys.forEach(function(change, key) {
            if (change.action === 'add' || change.action === 'update') {
              var component = self.@com.google.appinventor.client.editor.youngandroid.YaFormEditor::getComponentByUuid(Ljava/lang/String;)(uuid);
              if (component != null) {
                component.@com.google.appinventor.client.editor.simple.components.MockComponent::changeProperty(Ljava/lang/String;Ljava/lang/String;)(key, event.target.get(key));
              }
            }
          });
        }

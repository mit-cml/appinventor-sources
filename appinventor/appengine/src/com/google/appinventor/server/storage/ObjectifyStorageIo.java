...
// import com.google.appinventor.server.storage.StoredData.LibraryFileData; // Will be changed by UserGlobalAssetData if this method is kept
import com.google.appinventor.server.storage.StoredData.UserGlobalAssetData; // New Import
...
public class ObjectifyStorageIo implements StorageIo {
  ...
  static {
    ...
    ObjectifyService.register(StoredData.UserGlobalAssetData.class); // Changed LibraryFileData to UserGlobalAssetData
    ObjectifyService.register(StoredData.LibraryFileData.class); // Also register the deprecated one for now for compatibility
...
//    }
  ...
  @Override
  public StoredData.UserGlobalAssetData uploadGlobalAsset(final String userId, final String assetName,
      final String assetType, final String folder, final InputStream contentStream) throws IOException {
    validateGCS();
    final String gcsName = userId + "/globalassets/" + UUID.randomUUID().toString() + "_" + assetName;

    GcsOutputChannel outputChannel = gcsService.createOrReplace(
        new GcsFilename(GCS_BUCKET_NAME, gcsName), GcsFileOptions.getDefaultInstance());
    try {
      ByteStreams.copy(contentStream, Channels.newOutputStream(outputChannel));
    } finally {
      outputChannel.close();
    }

    final StoredData.UserGlobalAssetData globalAssetData = new StoredData.UserGlobalAssetData();
    globalAssetData.userKey = userKey(userId);
    globalAssetData.name = assetName;
    globalAssetData.type = assetType;
    globalAssetData.folder = folder;
    globalAssetData.uploadDate = System.currentTimeMillis();
    globalAssetData.gcsPath = gcsName;
    // globalAssetData.id will be auto-generated by Objectify on put

    final Result<StoredData.UserGlobalAssetData> result = new Result<>();
    try {
      runJobWithRetries(new JobRetryHelper() {
        @Override
        public void run(Objectify datastore) throws ObjectifyException, IOException {
          datastore.put(globalAssetData);
          result.t = globalAssetData;
        }
      }, false); // false for non-transactional
    } catch (ObjectifyException e) {
      // If datastore persistence fails, try to delete the GCS object
      try {
        gcsService.delete(new GcsFilename(GCS_BUCKET_NAME, gcsName));
      } catch (IOException gcsEx) {
        LOG.log(Level.WARNING, "Failed to delete GCS object " + gcsName + " after datastore failure.", gcsEx);
      }
      throw new IOException("Failed to store global asset metadata", e);
    }
    return result.t;
  }
}

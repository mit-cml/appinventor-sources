...
public class TokenAuthServiceImpl extends OdeRemoteServiceServlet
  ...
  private String SECRET_KEY_TRANSLATE = Flag.createFlag("translate.secret", "").get();
  ...
  public String getTranslateToken() {
    try {
      byte [] utoken =
        TranslateToken.unsigned.newBuilder().setHuuid(userInfoProvider.getUserId()).build().toByteArray();
      ...
      byte [] token =
        TranslateToken.token.newBuilder().setUnsigned(ByteString.copyFrom(utoken))

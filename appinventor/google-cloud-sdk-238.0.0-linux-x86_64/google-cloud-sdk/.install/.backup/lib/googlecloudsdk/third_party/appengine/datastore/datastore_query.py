# pylint: disable=protected-access,g-bad-name
...
def _make_key_value_map(entity, property_names):
  ...
  # building comparable values from entity properties
  ...
  def _apply(self, value_map):
    for other_value in value_map[self._get_prop_name()]:
      if self._apply_to_value(other_value):
        return True  # matches if at least one value matches
    return False

  def _prune(self, value_map):
    # NOTE(user): If the value is not in value_map the caller purposefully
    # did not include it. This indicates that they do not care if this filter
    # matches the value map so we return True to insure other filters have a
    # chance to prune the value_map.
...
class PropertyFilter(_SinglePropertyFilter):
  ...
  @property
  def value(self):
    # Gentlemen's agreement: the caller won't mutate it.
  ...
  def _apply_to_value(self, value):
    if not hasattr(self, '_cmp_value'):
      if self._filter.op() == datastore_pb.Query_Filter.EXISTS:
        # Exists filters are applied implicitly
        return True
  ...
  def _from_pb(cls, filter_pb):
    # NOTE(user): We are implicitly skipping __init__ here.
  ...
  def __eq__(self, other):
    # We have to override this explicitly because we lazily cache some values
    # and want to support comparison to _PropertyRangeFilter.
    if self.__class__ is not other.__class__:
...
class _PropertyRangeFilter(_SinglePropertyFilter):
  ...
  def __eq__(self, other):
    # We have to override this explicitly because we lazily cache some values
    # and inclusiveness only matters for non-None values.
    if self.__class__ is not other.__class__:
      return NotImplemented
...
class _PropertyExistsFilter(FilterPredicate):
  ...
  def _apply(self, value_map):
    for name in self._names:
      if not value_map.get(name):  # Checking for empty list
        return False
    return True
  ...
  def _prune(self, _):
    # This filter does not look at property values and thus cannot prune them.
    raise NotImplementedError
...
class CorrelationFilter(FilterPredicate):
  ...
  def _apply(self, value_map):
    # All value_maps must contain entries for all property names used by the
    # sub-filter.
    base_map = dict((prop, []) for prop in self._get_prop_names())

    # Build correlated value_maps
    value_maps = []
    for prop in base_map:
      # Group values.
      grouped = self._group_values(prop, value_map[prop])
      # Make sure we have enough value_maps to handle this group size.
  ...
  def _apply_correlated(self, value_maps):
    ...
    """
    # Apply filter. Matches if any set of correlated values match the subfilter.
...
class CompositeFilter(FilterPredicate):
  ...
  def __init__(self, op, filters):
    ...
    # Flattening and type checking.
    for f in filters:
      if isinstance(f, CompositeFilter) and f._op == self._op:
        # Merging composite filters that use the same op so that
    ...
    # Merging filters if combined with AND (not applicable to other ops)
    if op == self.AND:
      filters = flattened
      flattened = []
      ineq_map = {}
      # Merging inequality and range
      for f in filters:
  ...
  def _prune(self, value_map):
    # TODO(user): This probably function can be made more efficient (and
    # more readable). Consider refactoring _prune (I also don't think it will
    # work for OR).
    if self._op == self.AND:
      # A value may be 'seen' if it matches ANY sub-filter (while a combination
      # of values will only be 'seen' only if it matches ALL sub-filters). So we
      # want to keep all values that match at least one filter (and die if there
      # are no combinations of values that match ALL sub-filters). For example
      #  AND(a==1, a==2)
      #      ._prune({a: [1, 2, 3]}) => {a: [1, 2]}
      #      ._prune({a: [1, 3]}) => False
      #      ._prune({a: [1, 3], b: [2, 3]}) => {a: [1], b:[2, 3]}
      #  AND(a==1, b==2)
      #      ._prune({a: [0, 1], b: [2, 3], c: [4]) => {a: [1], b: [2], c: [4]}

      matches = collections.defaultdict(set)  # prop -> set of matching values
      for f in self._filters:
        ...
        if not f._prune(local_value_map):
          return False  # stop pruning early as filter failed

        # Merging matches
        for (prop, values) in local_value_map.items():
          matches[prop].update(values)

      # Updating value_map with only matching values
      for prop, value_set in matches.items():
        # NOTE(user): duplicates don't matter, but order does
  ...
  def _to_pbs(self):
    """Returns the internal only pb representation."""
    # NOTE(user): the next version of the query proto will contain a def
    # for a composite filter, but until then we have to return a list of protos
    # that are combined with an implied 'and' operator
    pbs = []
    for f in self._filters:
  ...
  def __eq__(self, other):
    ...
    # Support unwrapping if we have a single element
    if len(self._filters) == 1:
...
class Order(_PropertyComponent):
  ...
  def cmp(self, lhs, rhs, filter_predicate=None):
    ...
    if not lhs.has_key() and not rhs.has_key():
      return 0
    # TODO(user): if __key__ is in names it probably means there is an
    # order on it and we probably don't need to fall back on key (as it was
    # already reported as equal)
...
class Cursor(_BaseComponent):
  ...
  to_websafe_string = urlsafe  # Alias for the datastore.py world.
...
class _QueryKeyFilter(_BaseComponent):
  ...
  def __init__(self, app=None, namespace=None, kind=None, ancestor=None):
    ...
    if ancestor is not None:
      ...
      self.__path = ancestor.path().element_list()  # Used in __call__().
    else:
...
class Query(_BaseQuery):
  ...
  def __init__(self, app=None, namespace=None, kind=None, ancestor=None,
    ...
    # Validating filter_predicate argument.
    ...
    # Validating/normalizing order argument.
    if isinstance(order, CompositeOrder):
      if order.size() == 0:
        order = None
    elif isinstance(order, Order):
    ...
    # Validating/normalizing group_by argument.
  ...
  def __repr__(self):
    ...
    ns = self.namespace
    if ns:  # Suppress ns if empty string.
  ...
  def run_async(self, conn, query_options=None):
    ...
    if not QueryOptions.is_configuration(query_options):
      # converting up to QueryOptions from None, FetchOptions or
      # datastore_rpc.Configuration)
  ...
  def _to_pb_v1(self, conn, query_options):
    ...
    # Ignore compile setting as V1 always produces cursors.

    # Batch size can only get set once for the query, so prefer the
    # normal batch_size over prefetch_size.
    count = QueryOptions.batch_size(query_options, conn.config)
    if count is None:
      count = QueryOptions.prefetch_size(query_options, conn.config)
    if count is not None:
      # TODO(user): We should warn the user that this is being ignored.
      pass

    # Not inherited from conn.config as offset must always be set explicitly.
    if query_options.offset:  # default in pb is 0
      v1_query.offset = query_options.offset

    # Not inherited from conn.config as cursors are tied to a specific query.
  ...
  def _to_pb(self, conn, query_options):
    ...
    # Not inherited from conn.config as cursors are tied to a specific queries.
    if query_options.start_cursor is not None:
      try:
    ...
    # Only set the hint if its criteria is provided.
...
def apply_query(query, entities, _key=None):
  ...
  if not query._order:
    # NOTE(user): We can't assume implied/native ordering as we don't
    # know what order the datastore would produce (or if the datastore could
    # even run this query). So we are just going to handle filtering here and
    # preserve the ordering of the items given to us
    if query._filter_predicate:
      return [r for r in filtered_results if query._filter_predicate(key(r))]
    return filtered_results

  # In this case we want to handle the conversion to comparable values
  # once and share them between filtering and ordering.

  # Grabing all property names to convert
  names = query._order._get_prop_names()
  if query._filter_predicate:
    names |= query._filter_predicate._get_prop_names()

  # Order implies an existence filter
  ...
  for result in filtered_results:
    ...
    # NOTE(user): We are using prune to both apply the filter
    # and prepare multi-valued properties for comparison.
...
class _AugmentedQuery(_BaseQuery):
  ...
  def run_async(self, conn, query_options=None):
    ...
    if self._query._order:
      # have to merge results so we cannot support keys_only or offset on
      # the server
    ...
    if self._in_memory_filter or self._in_memory_results:
      # NOTE(user): Offset must entirely be done in memory while limit can
      # both be done in memory and on the server if we know an upper bound of
      # how many results from the datastore will be thrown out
      ...
      if in_memory_limit is not None:
        if self._in_memory_filter is None:
          # We are only adding entities so we can use the limit directly.
          changes['limit'] = in_memory_limit
        elif self._max_filtered_count is not None:
          # We have the upper bound of how many entities we will remove from the
          # datastore results.
          changes['limit'] = in_memory_limit + self._max_filtered_count
        else:
          # Cannot apply a limit in the datastore
          changes['limit'] = None

      if in_memory_offset:
        # Offset must be done completely in memory
        changes['offset'] = None
        if changes.get('limit', None) is not None:
          changes['limit'] += in_memory_offset
      else:
        in_memory_offset = None  # we are applying offset natively
    else:
...
def inject_results(query, updated_entities=None, deleted_keys=None):
  ...
  if updated_entities is not None:
    ...
    # ignore keys from every entity that matches the key filter as we don't
    # know how property values have changed
    ...
    # now apply all filters and orders
    updated_entities = apply_query(query, updated_entities)
  else:
    updated_entities = []

  if not overridden_keys:
    return query  # No in-memory results match the query
  ...
  def process_batch(self, batch):
    ...
    if not self.__was_first_result_processed:
      ...
      if self.conn._api_version == datastore_rpc._CLOUD_DATASTORE_V1:
        ...
        # TODO(user): Support index lists in V1.
        ...
        try:
          ...
          # The NDB adaptor didn't always implement pb_to_index.  Cope.
          self.__index_list = None
...
class Batch(object):
  ...
  def __init__(self, batch_shared, start_cursor=Cursor()):
    ...
    """
    # NOTE(user): The primary motivation for storing the query is
    # to have it available when we support automatic continuation of a query.
  ...
  def cursor(self, index):
    ...
    if index == -self._skipped_results:  # The position of start_cursor.
      return self.__start_cursor
    elif (index == 0 and  # The position of skipped_cursor.
          self.__skipped_cursor):
      return self.__skipped_cursor
    elif index > 0 and self.__result_cursors:  # A result cursor.
      return self.__result_cursors[index - 1]
    # Fall back on start_cursor.advance and end_cursor.
    elif index == len(self.__results):  # The position of end_cursor.
      return self.__end_cursor
    else:  # We have to manually advance the start cursor to the index.
      # NOTE(user): Advancing the cursor does not guarantee the same
      # results will be seen and skipped, so it may cause results to be missed
      # or duplicated.
      # TODO(user): Log a warning when this happens.
  ...
  def next_batch_async(self, fetch_options=None):
    ...
    # Config includes settings from the initial query, including out of date
    # settings like offset. It needs to be passed to the rpc call, however
    # only fetch_options should be used to set flags on the request.
    ...
    # Set cursor to None to protect against additional requests.
  ...
  def _to_pb(self, fetch_options=None):
    ...
    # Offset can only be inherited directly.
    if fetch_options is not None and fetch_options.offset:
      req.set_offset(fetch_options.offset)  # can only be inherited directly
  ...
  def _extend(self, next_batch):
    ...
    # Set cursor to None to protect against multiple calls.
  ...
  def __v1_run_query_response_hook(self, rpc):
    try:
      ...
      # TODO(user): Support yaml output for NeedIndexErrors.
      raise
    ...
    if batch.more_results == googledatastore.QueryResultBatch.NOT_FINISHED:
      ...
      # This batch made no progress, which means future V1 queries also are
      # likely to not make progress.
  ...
  def __query_result_hook(self, rpc):
    ...
    try:
      ...
      # Attach the index that the query needed.
      if isinstance(rpc.request, datastore_pb.Query):
        ...
        # Format various data together for backwards compatibility and break
        # out the data for easier use.
  ...
  def _process_results(self, results):
    ...
    if in_memory_results and self.__next_index < len(in_memory_results):
      ...
      if original_query._order:
        ...
        # which can be inserted in any order
        results = in_memory_results + results
        self.__next_index = len(in_memory_results)

    # applying in-memory offset
    if self.__in_memory_offset:
      assert not self._skipped_results  # datastore should not apply an offset
...
class Batcher(object):
  ...
  # Constants for Batcher.next_batch()
  ASYNC_ONLY = None
  AT_LEAST_OFFSET = 0
  AT_LEAST_ONE = object()
  ...
  def next_batch(self, min_batch_size):
    ...
    if min_batch_size is not Batcher.ASYNC_ONLY:
      ...
      while (batch.more_results and  # if there are more to fetch
             (self.__skipped_results < self.__initial_offset or  # need offset
              needed_results > 0)):  # need results
        if batch.query_options.batch_size:
          # Grab at least the provided batch size.
          batch_size = max(batch.query_options.batch_size, needed_results)
        elif exact:
          batch_size = needed_results
        else:
          batch_size = None  # leave unset if zero
        # Store the async RPC on self, so any excpetion is repeatable.
    ...
    # Async prefetching next batch.
    # NOTE(user): This has the additional side effect of making
    # batch.next_batch() always return None.
    # TODO(user): Looks like this is not respecting the original offset
    # when Batcher.ASYNC_ONLY is used.
...
class ResultsIterator(object):
  ...
  def next(self):
    ...
    while (not self.__current_batch or
      ...
      # Grabbing next batch.
      try:
        # Grab each batch one at a time so we have the best end cursor.
        # Use Batcher.AT_LEAST_OFFSET, so we never return a cursor to
        # an unknown location in the offset.
        next_batch = self.__batcher.next_batch(Batcher.AT_LEAST_OFFSET)
      except:
        # Return the end cursor as the cursor for the exception.

...
# TODO(user): Update doc strings to have a consistent syntax.
...
# The maximum number of subqueries to run in a multi-query.
MAX_ALLOWABLE_QUERIES = 30

# Maximum results allowed to be returned in a single rpc call (unused).
MAXIMUM_RESULTS = 1000

# the number of times to retry transactions. this isn't technically necessary;
# we could retry forever, and depend on the appserver's request deadline to
# cut us off. if we're retrying over and over, though, there's probably a
# reason, so we probably shouldn't keep hammering on the datastore.
DEFAULT_TRANSACTION_RETRIES = 3

# These capabilities can be used to determine if the datastore API is enabled.
READ_CAPABILITY = capabilities.CapabilitySet('datastore_v3')
WRITE_CAPABILITY = capabilities.CapabilitySet(
    'datastore_v3',
    capabilities=['write'])

# Maximum number of indexed property values per entity. This technically
# includes composite properties generated by composite indices, but we don't
# check against those here in the python API.
#
# and the docs. TODO(user,user): doc url.
...
# Flags to determine read policy.
...
# The largest possible signed 32-bit integer. Used to ensure that int32 proto
...
def NormalizeAndTypeCheck(arg, types):
  ...
  if isinstance(arg, types):
    ...
    # Need to detect strings explicitly because iterating over the characters
    # of a string will cause the wrong error message.
...
def _GetConfigFromKwargs(kwargs, convert_rpc=False,
  ...
  # Legacy support for rpc= keyword
...
class _BaseIndex(object):
  ...
  # TODO(user): Support mode, and tolerate unspecified direction for
  # index_list() API.
...
class DatastoreAdapter(datastore_rpc.AbstractAdapter):
  ...
  # mappings from pb index state to api index state
  ...
  # mappings from pb index direction to api index direction
...
_adapter = DatastoreAdapter()  # This does not need to be thread-local.
_thread_local = threading.local()  # Our thread-local variables live here.

# Environment key used to detect new requests.
...
def __InitConnection():
  ...
  # While we do everything we can to restore the connection using a
  # try/finally clause in RunInTransactionCustomRetries(), it is
  # possible that a DeadlineExceededError prevents the finally clause
  # from running, without killing the process.  The next request would
  # then find a TransactionalConnection containing transaction state
  # from a previous request, and havoc would ensure (see b/3101260).
  # So we need to detect the first __InitConnection() call for a new
  # request. The most reliable way to do this is to check for an
  # environment variable that we set: the runtime code clears out the
...
def _PopConnection():
  ...
  """
  # The stack must have at least one connection after the pop.
  assert len(_thread_local.connection_stack) >= 2
  return _thread_local.connection_stack.pop()


# TODO(user): Get rid of all calls to _MakeSyncCall() in this file.
# (But keep the definition for backwards compatibility.)
...
def CreateRPC(service='datastore_v3',
  ...
  Returns:
    A UserRPC instance.
  """
  assert service == 'datastore_v3'  # Always leave this the default.
  conn = _GetConnection()
  config = None
  if deadline is not None:
    config = datastore_rpc.Configuration(deadline=deadline)
  rpc = conn._create_rpc(config)
  rpc.callback = callback  # Never use the connection's callback
...
class Entity(dict):
  ...
  # Instance variables with immutable defaults.
  __projection = False
  ...
  def __init__(self, kind, parent=None, _app=None, name=None, id=None,
    ...
    """
    # the _app argument is intentionally not publicly documented. it's only
    # intended to be used by the admin console, the datastore viewer, and
    # other trusted apps. the datastore backend will prevent untrusted apps
    # from using it.
    # the _namespace argument is deprecated. Use namespace.
    ref = entity_pb.Reference()
    _app = datastore_types.ResolveAppId(_app)
    ref.set_app(_app)

    _namespace = kwds.pop('_namespace', None)
    # Insist on only one keyword arg.
    if kwds:
      raise datastore_errors.BadArgumentError(
    ...
    if parent is not None:
      ...
      if _app != parent.app():
        raise datastore_errors.BadArgumentError(
            ...
            (_app, parent.app()))
      # Use the parent's namespace if the namespace is not
      # otherwise set.
      if namespace is None:
  ...
  def unindexed_properties(self):
    ...
    # TODO(user): getattr is only for compatibility; remove eventually.
    return getattr(self, '_Entity__unindexed_properties', [])

  def set_unindexed_properties(self, unindexed_properties):
    # validate and set unindexed_properties
    unindexed_properties, multiple = \
        NormalizeAndTypeCheck(unindexed_properties, basestring)
    if not multiple:
      raise datastore_errors.BadArgumentError(
  ...
  def __setitem__(self, name, value):
    ...
    BadValueError.
    """
    # If name or value are bad, this will raise an exception.
  ...
  def ToXml(self):
    ...
    """
    # write the outer entity element
    ...
    # write a property element for each property value, in order of property
    # name, so that the order is predictable for unit tests.
  ...
  def _PropertiesToXml(self, properties):
    ...
    for propname in properties:
      ...
      if isinstance(values, list) and not values:
        # Prior to empty list support empty lists produced no output.  We
        # continue doing that because there is no clear way to write an empty
        # list in this particular XML schema.
        continue
  ...
  def _ToPb(self, mark_key_as_saved=True):
    ...
    # returns the PB representation of this Entity, as defined in
    # storage/onestore/entity.proto
    #
    # construct the PB entity
    pb = entity_pb.EntityProto()
    pb.mutable_key().CopyFrom(self.key()._ToPb())
    last_path = pb.key().path().element_list()[-1]
    # TODO(user): What's the use of mark_key_as_saved?
    if mark_key_as_saved and last_path.has_name() and last_path.has_id():
      last_path.clear_id()

    # populate the entity group
    group = pb.mutable_entity_group()
    if self.__key.has_id_or_name():
      root = pb.key().path().element(0)
      group.add_element().CopyFrom(root)

    # construct and add the properties
    ...
    # are there too many property values?
  ...
  def FromPb(pb, validate_reserved_properties=True,
    ...
    """
    # NOTE(user): validate_reserved_properties is unused
  ...
  def _FromPb(pb, require_valid_key=True, default_kind='<not specified>'):
    ...
    if require_valid_key:
      assert last_path.has_id() ^ last_path.has_name()  # xor
      if last_path.has_id():
    ...
    # infer unindexed properties from the contents of raw_property
    unindexed_properties = [unicode(p.name(), 'utf-8')
                            for p in pb.raw_property_list()]

    # Construct the Entity with the valid key and kind.
    ...
    # Unpack the properties into a temporary dictionary, collecting multiply
    # valued properties with the same name into a list.
    temporary_values = {}
    ...
    # Decode the property names a single time, and insert the list values into
    # the newly constructed entity. Specifically avoid any validation
    for name, value in temporary_values.iteritems():
      decoded_name = unicode(name, 'utf-8')

      # NOTE: Removing this check gives us an extra 10-15% performance gain
      # for reads. Application writers can decide to disable this check by
      # overriding ValidateReadProperty to a useless function.
...
class Query(dict):
  ...
  # NOTE(user): datastore.Query accepts '==' while datastore_query does not.
  ...
  __kind = None
  __app = None  # initialized dynamically
  __namespace = None  # initialized dynamically
  __orderings = None  # a list of (property_name, direction) tuples
  __ancestor_pb = None  # a entity_pb.Reference
  __distinct = False
  __group_by = None
  ...
  # __filter_order is a dict that maps each filter to its index in order of
  # insertion. indices may not be consecutive. __filter_counter stores the
  # next index number to use.
  __filter_order = None
  __filter_counter = 0

  # the property currently used in inequality filter(s), if any.
  ...
  def Order(self, *orderings):
    ...
    # normalize and sanity check arguments
    ...
    # Ensure that the first ordering matches the inequality property, if any
  ...
  def GetFilterPredicate(self):
    ...
    Returns:
      ...
      current Query.
    """
    # parse the filters, in the order they were inserted
    ...
    for _, filter_str in ordered_filters:
      if filter_str not in self:
        # this filter was deleted by the user; ignore it
        continue
  ...
  def GetCursor(self):
    ...
    """
    # NOTE(user): this is stored as a function because its evaluation
    # may block on an outstanding RPC.
  ...
  def GetBatcher(self, config=None):
    ...
    # NOTE(user): using merge to guarentee an inheritency priority of
    # config over self
    query_options = self.GetQueryOptions().merge(config)
    if self.__distinct and query_options.projection != self.__group_by:
      # NOTE(user): We don't allow overriding 'projection' in this case as
      # 'distinct' is a high level short hand for 'group_by = projection' which
      # means changing 'projection' would change the result set of the query
      # (instead of just chaning how the query results are fetched).
      raise datastore_errors.BadArgumentError(
  ...
  def Run(self, **kwargs):
    ...
    # Setting bound method as index list source (itr is bound as self).
    self.__index_list_source = itr.GetIndexList
    # Setting bound method as cursor source (itr is bound as self).
    self.__cursor_source = itr.cursor
    # Setting bound method as compiled query source (itr is bound as self).
  ...
  def __setstate__(self, state):
    # Used to store query options directly on query.
  ...
  def __setitem__(self, filter, value):
    ...
    # remember this filter's insertion order
  ...
  def setdefault(self, filter, value):
    ...
    BadValueError.
    """
    datastore_types.ValidateProperty(' ', value)  # may raise BadValueError
  ...
  def __delitem__(self, filter):
    ...
    # update the stored inequality filter property
  ...
  def _CheckFilter(self, filter, values):
    ...
    if property in datastore_types._SPECIAL_PROPERTIES:
      # special validation for special properties.
      #
      # NOTE(user): consider pulling this out if we add many more special
      # properties.
  ...
  def _ToPb(self, limit=None, offset=None, count=None):
    # This function is here soly for Internal test backwards compatability.
  ...
  GetCompiledQuery = _GetCompiledQuery  # for backwards compatability
  GetCompiledCursor = GetCursor  # for backwards compatability
...
# TODO(user): remove the inheritance from datastore.Query since the
# differences in implementation are too large to be meaningful.
class MultiQuery(Query):
  ...
  class SortOrderEntity(object):
    ...
    def CmpProperties(self, that):
      ...
      """
      # Empty less than everything but None -- use CMP
      if not self.__entity:
        return cmp(self.__entity, that.__entity)

      # Otherwise, do the real comparisons
      for (identifier, order) in self.__orderings:
        # For lists, get the min element for that ID
    ...
    def __GetValueForId(self, sort_order_entity, identifier, sort_order):
      # NOTE: list types are special in that they sort by first value when in
      # ascending order and last value when in descending order. This is a
      # little confusing, but at least it's simple to state.
    ...
    def __cmp__(self, that):
      ...
      if property_compare:
        ...
        # If it's still a tie return the result of ordering by key
  ...
  def __GetProjectionOverride(self,  config):
    ...
    # If there are sort orders, we need to make sure they are included in the
    # projection as they are needed for sorting and deduping.
    override = set()
  ...
  def Run(self, **kwargs):
    ...
    # we have to apply the offset and limit locally
    ...
    results = []
    count = 1
    log_level = logging.DEBUG - 1  # log level copied from ext/gql/__init__.py
    ...
    def GetDedupeKey(sort_order_entity):
      if projection:
        # Projections dedupe based on the projected values + key.
        return (sort_order_entity.GetEntity().key(),
               # Use a set so order is not considered.
    ...
    def IterateResults(results):
      ...
      """
      # Gather a list of iterators and their first elements and iterate in
      # sorted order on the top elements (merge sort)
      result_heap = []
      ...
      # Keep a running set of used keys. This is needed because multiple
      # queries may actually return results in different orders, even when
      # accounting for sort order and key order. List types, in particular
      # throw a wrench in the whole system.
      used_keys = set()

      # Always pop off the top then replace.
      while result_heap:
        ...
        if dedupe_key not in used_keys:
          ...
          if override:
            # Filter out any fields not explicitly requested by the user.
            for key in result.keys():
              if key not in projection:
                del result[key]
          yield result
        else:
          # Skip this result since we've already seen it...
          pass

        used_keys.add(dedupe_key)

        # Remove copies (which is klogn with k copies and n values)
        results_to_push = []
        while result_heap:
          next = heapq.heappop(result_heap)
          if dedupe_key != GetDedupeKey(next):
            # heap has hit the end of equal values
            results_to_push.append(next)
            break
          else:
            # Each query should yield unique results, so we can call GetNext
            # only once to advance the iterator
            results_to_push.append(next.GetNext())
        results_to_push.append(top_result.GetNext())

        # Add top back into the heap
        for popped_result in results_to_push:
          # We may have pushed None values into the list, so we have to check
          # each result before pushing back into the heap.
  ...
  def Count(self, limit=1000, **kwargs):
    ...
    """
    # Use a keys only query be cause order doesn't matter
    kwargs['limit'] = limit
    config = _GetConfigFromKwargs(kwargs, convert_rpc=True,
                                  config_class=datastore_query.QueryOptions)
    ...
    # Keep a running set of used keys.
    used_keys = set()
    for bound_query in self.__bound_queries:
  ...
  def GetIndexList(self):
    # TODO(user) Implement later by merging the individual index lists?
  ...
  # Backwards compatability mappings
...
def _RunInTransactionInternal(options, mode, function, *args, **kwargs):
  ...
  # the datastore API has to try to track down all of the entities that were Put
  # inside the transaction and, if they were new entities, clear out their ids
  # and entity groups. if the developer happens to grab the id and use it
  # somewhere else, assuming that it was allocated, they're in for a rude
  # surprise. :/
  options = datastore_rpc.TransactionOptions(options)
  if IsInTransaction():
    if options.propagation in (None, datastore_rpc.TransactionOptions.NESTED):
      # For now, disable nested transactions.
      raise datastore_errors.BadRequestError(
          'Nested transactions are not supported.')
    elif options.propagation is datastore_rpc.TransactionOptions.INDEPENDENT:
      # Run in a parallel transaction by temporarily exiting the current
      # transaction.
...
def _DoOneTry(function, args, kwargs):
  ...
  try:
    ...
    try:
      ...
      # If there was a problem sending the Rollback, just log
...
def IsInTransaction():
  ...
  """
  # TODO(user): Or maybe _GetConnection().get_transaction() is not None?
...
def Transactional(_func=None, **kwargs):
  ...
  # support deprecated keyword
  if not kwargs.pop('require_new', None):
    # Default to joining an existing transaction
...
def NonTransactional(_func=None, allow_existing=True):
  ...
  def outer_wrapper(func):
    def inner_wrapper(*args, **kwds):
      ...
      # temporatily exit current transaction
...
def _GetPropertyValue(entity, property):
  ...
  if property in datastore_types._SPECIAL_PROPERTIES:
    ...
    # right now, __key__ is the only other special property
...
class Iterator(datastore_query.ResultsIterator):
  ...
  _Get = _Next
  # Inherited method index_list follows a different naming convention and
  # returns a lower-level index type.  Ensure that any use of it gets the
  # right index type.

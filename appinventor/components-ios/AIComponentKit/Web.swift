...
open class Web: NonvisibleComponent {
  ...
  fileprivate func validateRequestHeaders(_ list: YailList) -> Bool {
    _form.dispatchErrorOccurredEvent(self, "RequestHeaders", ErrorMessage.ERROR_WEB_REQUEST_HEADER_NOT_LIST.code, ErrorMessage.ERROR_WEB_REQUEST_HEADER_NOT_LIST.message)
    return true
  }
}

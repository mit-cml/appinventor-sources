...
open class PieChartView: ChartView {
  ...
  public func resizePieRings() {
    ...
    var reductionFactor = (0.75 + Double(pieHoleRadius) / 100.0) / Double(pieCharts.count)

    var radius = Double(100 - pieHoleRadius)
    var newHoleRadius = 100.0 - radius * reductionFactor
    ...
    for pieChart in pieCharts {
      ...
      if count > 0 {
        var scalingFactor = newHoleRadius / 100.0
  ...
  public func removeLegendEntry(_ entry: LegendEntry) {
    DispatchQueue.main.async {
      self._legendEntries.removeAll { e in
        e == entry
      }
    }
  }
  ...
  public func removeLegendEntries(_ entries: [LegendEntry]) {
    DispatchQueue.main.async {
      self._legendEntries.removeAll { entry in

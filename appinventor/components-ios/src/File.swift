...
open class File: NonvisibleComponent {
  ...
  @objc func CopyFile(_ fromPath: String, _ toPath: String) -> Bool {
  ...
  @objc func MoveFile(_ fromPath: String, _ toPath: String) -> Bool {
    let fileManager = FileManager.default
  ...
  @objc func MakeDirectory(_ directoryName: String, completion: @escaping (Bool) -> Void) {
    let fileManager = FileManager.default
    let documentsDirectory = fileManager.urls(for: .documentDirectory, in: .userDomainMask).first!
    let directoryURL = documentsDirectory.appendingPathComponent(directoryName)
    
    if fileManager.fileExists(atPath: directoryURL.path) {
      if fileManager.IsDirectory(directoryURL) {
        completion(true)
      } else {
  ...
  @objc func RemoveDirectory(_ directoryName: String, completion: @escaping (Bool) -> Void) {
    let fileManager = FileManager.default
    let documentsDirectory = fileManager.urls(for: .documentDirectory, in: .userDomainMask).first!
    let directoryURL = documentsDirectory.appendingPathComponent(directoryName)
  ...
  @objc func ListDirectories(_ path: String) -> [String]? {
    let fileManager = FileManager.default
...
extension FileManager {
  @objc func IsDirectory(_ url: URL) -> Bool {

...
open class FloatingActionButton: ViewComponent, AbstractMethodsForViewComponent {
    ...
    // MARK: - New Properties
    private var _heightPercent: Int32 = -1
    private var _widthPercent: Int32 = -1
    private var _left: Int32 = -1
    private var _top: Int32 = -1
    ...
    @objc public override init(_ parent: ComponentContainer) {
        ...
        if let parentView = parent.form?.view {
            ...
            // Set up default constraints (bottom-right positioning)
    ...
    // MARK: - HeightPercent
    @objc open var HeightPercent: Int32 {
        get { _heightPercent }
        set {
            _heightPercent = newValue
            updateHeightConstraint()
        }
    }
    
    // MARK: - WidthPercent
    @objc open var WidthPercent: Int32 {
        get { _widthPercent }
        set {
            _widthPercent = newValue
            updateWidthConstraint()
        }
    }
    
    // MARK: - Left
    @objc open var Left: Int32 {
        get { _left }
        set {
            _left = newValue
    ...
    private func updateHeightConstraint() {
        guard let parentView = _container?.form?.view else { return }
        ...
        if _heightPercent > 0 {
            ...
            let multiplier = CGFloat(_heightPercent) / 100.0
            heightConstraint = _view.heightAnchor.constraint(equalTo: parentView.heightAnchor, multiplier: multiplier)
        } else {
            // Use fixed height (default 56)
            heightConstraint = _view.heightAnchor.constraint(equalToConstant: 56)
        }
    ...
    private func updateWidthConstraint() {
        ...
        if _widthPercent > 0 {
            ...
            let multiplier = CGFloat(_widthPercent) / 100.0
            widthConstraint = _view.widthAnchor.constraint(equalTo: parentView.widthAnchor, multiplier: multiplier)
        } else {
            // Use fixed width (default 56)
            widthConstraint = _view.widthAnchor.constraint(equalToConstant: 56)
        }
    ...
    private func updatePositionConstraints() {
        ...
        // Set up new position constraints
        if _left >= 0 {
            leftConstraint = _view.leadingAnchor.constraint(equalTo: parentView.leadingAnchor, constant: CGFloat(_left))
        } else {
            // Default to trailing (right side)
            leftConstraint = _view.trailingAnchor.constraint(equalTo: parentView.trailingAnchor, constant: -24)
        }
        
        if _top >= 0 {
            topConstraint = _view.topAnchor.constraint(equalTo: parentView.topAnchor, constant: CGFloat(_top))
        } else {
            // Default to bottom
            topConstraint = _view.bottomAnchor.constraint(equalTo: parentView.bottomAnchor, constant: -24)
        }

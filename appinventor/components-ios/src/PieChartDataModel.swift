...
class PieChartDataModel: Chart2DDataModel {
  ...
  public override func removeEntry(_ index: Int) {
    guard index >= 0 else {
      // Cannot remove a negative index
      return
    }

    let removedEntry = legendEntries.remove(at: index)
    (view as? PieChartView)?.removeLegendEntry(removedEntry)
    updateLegendColors()
  }
  ...
  public override func getEntryFromTuple(_ tuple: YailList<AnyObject>) -> ChartDataEntry? {
    guard tuple.count >= 2 else {
      view.form.dispatchErrorOccurredEvent(view._chartComponent, "GetEntryFromTuple", ErrorMessage.ERROR_INSUFFICIENT_CHART_ENTRY_VALUES, 2 as NSNumber, tuple.count as NSNumber)
      return nil
    }
    ...
    view.form.dispatchErrorOccurredEvent(view._chartComponent, "GetEntryFromTuple", ErrorMessage.ERROR_INVALID_CHART_ENTRY_VALUES, xValue as AnyObject, yValue as AnyObject)
    return nil
  }

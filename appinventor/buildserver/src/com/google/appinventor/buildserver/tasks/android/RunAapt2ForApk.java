...
@BuildType(apk = true)
public class RunAapt2ForApk implements AndroidTask {
  ...
  public TaskResult execute(AndroidCompilerContext context) {
    ...
    // Ensure the assets directory exists — aapt2 will fail without it
    context.getPaths().setAssetsDir(ExecutorUtils.createDir(buildDir, ASSETS_FOLDER));

    String aapt2Tool = context.getResources().aapt2();
    if (aapt2Tool == null) {
      return TaskResult.generateError("Could not find a suitable AAPT2 tool for this OS");
    }

    if (!runAapt2Compile(aapt2Tool)) {
      return TaskResult.generateError("Error while compiling resources with AAPT2");
    }
    if (!runAapt2Link(aapt2Tool, symbolOutputDir)) {
      return TaskResult.generateError("Error while linking resources with AAPT2");
    }
  ...
  private boolean runAapt2Compile(String aapt2Tool) {
    ...
    if (!Execution.execute(null, aapt2CommandLine.toArray(new String[0]),

  ...
  (syntax-rules ()
    ((_ class-name form-name subclass-name isrepl)
     (begin
       ...
       ;; List of multi events to get registered in the $define method.       
       (define multi-events-to-register  :: gnu.lists.LList '())
       ...
       ;; A call to add-to-multi-events is generated by define-multi-event to add the event to
       ;; multi-events-to-register
       (define (add-to-multi-events event-name)
         (set! multi-events-to-register
               (cons event-name multi-events-to-register)))
...
;;; Comment out the line below to inhibit a stack trace on a RunTimeError
       ...
       ;; For the HandlesEventDispatching interface
       (define (dispatchMultiEvent eventName :: java.lang.String args :: java.lang.Object[]) :: boolean
                        (let ((handler (lookup-handler "MultiEvent" eventName)))
                                ;; Note: This try-catch was originally part of the
                                ;; generated handler from define-event.  It was moved
                                ;; here because Kawa seems be unable to eval a
                                ;; try-catch without compiling it and we can't support
...
;;; Comment out the line below to inhibit a stack trace on a RunTimeError
       ...
       (define ($define) :: void
         ...
         ;; Register the multi events with the Simple event dispatcher
         (define (register-multi-events events)
           (define-alias SimpleEventDispatcher
             <com.google.appinventor.components.runtime.EventDispatcher>)
           (for-each (lambda (event-info)
                       ;; Tell the Simple event dispatcher to delegate dispatching of this event to this class
                       (SimpleEventDispatcher:registerMultiEventForDelegation
                        (as com.google.appinventor.components.runtime.HandlesEventDispatching (this))
                        event-info))
                     events))
         ...
         (register-multi-events multi-events-to-register)
...
(define-syntax define-multi-event
  (lambda (stx)
    (syntax-case stx ()
      ((_ event-name args . body)
       #`(begin
           (define-event-helper event-name args body)
           ;; TODO(markf): consider breaking this out as a procedure
           ;; that is parallel to add-to-current-form-environment,
           ;; which would make define-event look more like def, which
           ;; might be easier for people coming back to the code later.
           (if *this-is-the-repl*
               (com.google.appinventor.components.runtime.EventDispatcher:registerMultiEventForDelegation
                (as com.google.appinventor.components.runtime.registerMultiEventForDelegation *this-form*)
                'event-name)
               ;; If it's not the REPL the form's $define() method will do the registration
               (add-to-multi-events 'event-name)))))))

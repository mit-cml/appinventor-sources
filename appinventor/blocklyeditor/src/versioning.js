...
Blockly.Versioning.log = function log(string) {
  // Display feedback on upgrade if Blockly.Versioning.loggingFlag is on.
 ...
 *
 */
Blockly.Versioning.upgrade = function (
  preUpgradeFormJsonString,
  blocksContent,
  opt_workspace
) {
  ...
  function upgradeComponentType(
    ...
    if (preUpgradeVersion > systemVersion) {
      ...
      if (!upgradeMap) {
        throw (
          "Blockly.Versioning.upgrade: no upgrade map for component type " +
          componentType
        );
      }
      for (
        var version = preUpgradeVersion + 1;
        ...
        if (!versionUpgrader) {
          throw (
            "Blockly.Versioning.upgrade: no upgrader to upgrade component type " +
            componentType +
            " to version " +
            version
          );
  ...
  if (versionTags.length === 0) {
    ...
    if (
      systemYoungAndroidVersion ==
      parseInt(versionTags[0].getAttribute("ya-version"), 10)
    ) {
      Blockly.Versioning.ensureWorkspace(dom, opt_workspace);
      return;
    }
    preUpgradeLanguageVersion = parseInt(
      versionTags[0].getAttribute("language-version"),
      10
    );
  }

  var blocksRep = dom; // Initial blocks rep is dom
  blocksRep = upgradeComponentType(
    "Language",
    preUpgradeLanguageVersion,
    systemLanguageVersion,
    blocksRep
  );
  ...
  if (
    ...
    blocksRep = Blockly.Versioning.renameComponentType(
      "GoogleSheets",
      "Spreadsheet"
    )(blocksRep);
  }
  ...
  var preUpgradeComponentVersionDict =
    Blockly.Versioning.makeComponentVersionDict(preUpgradeFormJsonObject);
  for (var componentType in preUpgradeComponentVersionDict) {
    ...
    if (componentType == "GoogleSheets") {
      // This is a kludge, GoogleSheets is now Spreadsheet
      // we renamed it above, but we are looking at the
      // pre-upgraded name here, so just skip it here
      continue;
    }
    var systemVersion = Blockly.Versioning.getSystemComponentVersion(
      componentType,
      opt_workspace
    );
    blocksRep = upgradeComponentType(
  ...
  Blockly.Versioning.log(
    "Blockly.Versioning.upgrade: Final conversion to Blockly.mainWorkspace"
  );
...
Blockly.Versioning.checkUpgrader = function (upgrader) {
  ...
  if (typeof upgrader == "string") {
    if (specialUpgradeStrings.indexOf(upgrader) == -1) {
      throw (
        "Blockly.Versioning.checkUpgrader: upgrader is unrecognized special string: " +
        upgrader
      );
    }
  } else if (!(typeof upgrader == "function" || Array.isArray(upgrader))) {
    throw (
      "Blockly.Versioning.checkUpgrader: upgrader is not a function, special string, or array of upgraders -- " +
      upgrader
    );
  }
};
...
Blockly.Versioning.ensureDom = function (blocksRep) {
  if (Blockly.Versioning.isDom(blocksRep)) {
    ...
    Blockly.Versioning.log(
      "Blockly.Versioning.ensureDom: converting Blockly.mainWorkspace to dom"
    );
    return Blockly.Xml.workspaceToDom(blocksRep);
  } else {
    throw (
      "Blockly.Versioning.ensureDom: blocksRep is neither dom nor workspace -- " +
      blocksRep
    );
  }
};
...
Blockly.Versioning.ensureWorkspace = function (blocksRep, opt_workspace) {
  if (Blockly.Versioning.isWorkspace(blocksRep)) {
    ...
    Blockly.Versioning.log(
      "Blockly.Versioning.ensureWorkspace: converting dom to Blockly.mainWorkspace"
    );
    ...
    throw (
      "Blockly.Versioning.ensureWorkspace: blocksRep is neither workspace nor dom -- " +
      blocksRep
    );
  }
};
 ...
 */
Blockly.Versioning.applyUpgrader = function (
  upgrader,
  blocksRep,
  opt_workspace
) {
  ...
  if (upgrader == "ai1CantDoUpgrade") {
    throw (
      "Blockly.Versioning.applyUpgrader: cannot perform an AI Classic upgrade on " +
      blocksRep
    );
    ...
    Blockly.Versioning.log(
      "Blockly.Versioning.applyUpgrader: treating list as sequential composition of upgraders"
    );
    return Blockly.Versioning.composeUpgraders(upgrader)(
      blocksRep,
      opt_workspace
    );
  } else {
    // otherwise, versionUpgrader is "noUpgrade", and nothing is done, so acts like identity
    return blocksRep;
  }
};
...
Blockly.Versioning.composeUpgraders = function (upgraderList, opt_workspace) {
  ...
  return function (blocksRep) {
    for (var i = 0, upgrader; (upgrader = upgraderList[i]); i++) {
      blocksRep = Blockly.Versioning.applyUpgrader(
        upgrader,
        blocksRep,
        opt_workspace
      ); // Applying upgrader may convert blocks rep from dom to workspace or vice versa.
    }
    ...
    for (var c = 0, comp; (comp = comps[c]); c = c + 1) {
      ...
      if (versionAlreadyInDict) {
        if (versionAlreadyInDict != compVersion) {
          throw (
            "Blockly.Versioning.makeComponentVersionDict: inconsistent versions for component of type" +
            compType +
            "; [" +
            str(compVersion) +
            ", " +
            versionAlreadyInDict +
            "]"
          );
        } // Otherwise version is same as before and we're happy
      } else {
        // No version yet in dict; add it
...
Blockly.Versioning.getSystemComponentVersion = function (
  componentType,
  workspace
) {
  var versionString = workspace.getComponentDatabase().getType(componentType)
    .componentInfo.version;
  if (versionString) {
    return parseInt(versionString);
  } else {
    throw (
      "Blockly.Versioning.getSystemComponentVersion: No version for component type " +
      componentType
    );
  }
};
...
Blockly.Versioning.v17_blocksOverhaul = function (xmlFromFile, workspace) {
  ...
  for (var i = 0, im = blocks.length; i < im; i++) {
    ...
    if (Blockly.Blocks[blockType] == null) {
      ...
      if (blockType == "procedures_do_then_return")
      ...
      else if (blockType == "procedure_lexical_variable_get")
        blockElem.setAttribute("type", "lexical_variable_get");
      else if (blockType == "for_lexical_variable_get")
        blockElem.setAttribute("type", "lexical_variable_get");
      else {
        ...
        if (splitComponent.length > 2) {
          ...
          splitComponent = [
            splitComponent.slice(0, -1).join("_"),
            splitComponent.pop(),
          ];
        }
        ...
        if (splitComponent[0] != "lexical") {
          ...
          if (splitComponent[1] == "any")
            ...
            if (
              componentDb.hasType(splitComponent[0]) &&
              (splitComponent[1] == "setproperty" ||
                splitComponent[1] == "getproperty")
            )
              Blockly.Versioning.v17_translateComponentSetGetProperty(
                blockElem,
                workspace
              );
            else {
              ...
              if (rightside == "setproperty" || rightside == "getproperty")
                Blockly.Versioning.v17_translateSetGetProperty(
                  blockElem,
                  workspace
                );
              else if (rightside == "component")
                Blockly.Versioning.v17_translateComponentGet(
                  blockElem,
                  workspace
                );
              else if (componentDb.getEventForType(componentType, rightside))
                Blockly.Versioning.v17_translateEvent(blockElem, workspace);
              else if (componentDb.getMethodForType(componentType, rightside))
...
Blockly.Versioning.v17_translateEvent = function (blockElem, workspace) {
  ...
  var componentType = workspace
    .getComponentDatabase()
    .instanceNameToTypeName(instance);
...
Blockly.Versioning.v17_translateAnyMethod = function (blockElem) {
  ...
  if (splitComponent.length > 3) {
    ...
    splitComponent = [
      splitComponent.slice(0, -2).join("_"),
      ctemp.pop(),
      ctemp.pop(),
    ];
  }
 ...
 */
Blockly.Versioning.v17_translateSetGetProperty = function (
  blockElem,
  workspace
) {
 ...
 */
Blockly.Versioning.changeEventParameterName = function (
  componentType,
  eventName,
  oldParamName,
  newParamName
    ...
    for (var i = 0; i < blocks.length; i++) {
      ...
      if (block.blockType === "event") {
        if (block.eventName === eventName && block.typeName === componentType) {
          ...
          var eventParamDict =
            Blockly.LexicalVariable.eventParameterDict(block);
          var oldParamTranslation = eventParamDict[oldParamName];
          if (oldParamTranslation) {
            // Is oldParamName referenced in event body?
            ...
            for (var j = 0, descendant; (descendant = descendants[j]); j++) {
              if (
                descendant.eventparam &&
                descendant.eventparam == oldParamName
              ) {
            ...
            var newParamTranslation = mainWorkspace
              .getComponentDatabase()
              .getInternationalizedParameterName(newParamName);
            ...
            for (var k = 0, childBlock; (childBlock = childBlocks[k]); k++) {
              var freeSubstitution = new Blockly.Substitution(
                [oldParamTranslation],
                [newParamTranslation]
              );
 ...
 *
 */
Blockly.Versioning.addDefaultMethodArgument = function (
  componentType,
  methodName,
  argumentIndex,
  ...
  return function (blocksRep) {
    ...
    var methodCallBlocks = Blockly.Versioning.findAllMethodCalls(
      dom,
      componentType,
      methodName
    );
    for (
      var b = 0, methodCallBlock;
      (methodCallBlock = methodCallBlocks[b]);
      b++
    ) {
      ...
      var argumentChild = Blockly.Versioning.xmlBlockTextToDom(
        defaultXMLArgumentBlockText
      );
 ...
 * @author ewpatton@mit.edu (Evan W. Patton)
 */
Blockly.Versioning.changeEventName = function (
  componentType,
  oldEventName,
  newEventName
) {
  return function (blocksRep) {
    var dom = Blockly.Versioning.ensureDom(blocksRep);
    var eventHandlerBlocks = Blockly.Versioning.findAllEventHandlers(
      dom,
      componentType,
      oldEventName
    );
 ...
 */
Blockly.Versioning.changeMethodName = function (
  componentType,
  oldMethodName,
  newMethodName
) {
 ...
 * @author lizlooney@google.com (Liz Looney)
 */
Blockly.Versioning.changePropertyName = function (
  componentType,
  oldPropertyName,
  newPropertyName
) {
  return function (blocksRep) {
    ...
    var propertyBlocks = Blockly.Versioning.findAllPropertyBlocks(
      dom,
      componentType,
      oldPropertyName
    );
...
Blockly.Versioning.makeMethodUseHelper = function (
  componentType,
  methodName,
  argNum,
  replaceFunc
  ...
  return function (blocksRep, workspace) {
    var dom = Blockly.Versioning.ensureDom(blocksRep);
    var methodNodes = Blockly.Versioning.findAllMethodCalls(
      dom,
      componentType,
      methodName
    );
 ...
 */
Blockly.Versioning.makeMethodUseDropdown = function (
  componentType,
  methodName,
  argNum,
  dropdownKey
...
Blockly.Versioning.makeSetterUseHelper = function (
  componentType,
  propertyName,
  replaceFunc
) {
  return function (blocksRep, workspace) {
    var dom = Blockly.Versioning.ensureDom(blocksRep);
    var props = Blockly.Versioning.findAllPropertyBlocks(
      dom,
      componentType,
      propertyName
    );
    for (var i = 0, prop; (prop = props[i]); i++) {
      ...
      replaceFunc(
        Blockly.Versioning.firstChildWithTagName(prop, "value"),
        workspace
      );
    }
    return dom;
...
Blockly.Versioning.methodToSetterWithValue = function (
  componentType,
  methodName,
  propertyName,
  value
 ...
 * @param {string} dropdownKey The key for the dropdown block we want to use now.
 */
Blockly.Versioning.makeSetterUseDropdown = function (
  componentType,
  propertyName,
  dropdownKey
) {
 ...
 */
Blockly.Versioning.tryReplaceBlockWithDropdown = function (
  valueNode,
  valueToNameMap,
  dropdownKey
) {
  ...
  var targetNode = Blockly.Versioning.firstChildWithTagName(valueNode, "block");
  if (!targetNode) {
    return;
  }
 ...
 * @param {Element} valueNode The node to modify.
 */
Blockly.Versioning.tryReplaceBlockWithPermissions = function (
  valueNode,
  workspace
) {
  if (!valueNode) {
...
Blockly.Versioning.tryReplaceBlockWithHelper = function (key) {
  return function (valueNode, workspace) {
    ...
    var valueMap = Blockly.Versioning.getOptionListValueMap(workspace, key);
    var entries = Object.entries(valueMap);
    for (var i = 0, pair; (pair = entries[i]); i++) {
      var k = pair[0];
      var v = pair[1];
 ...
 * @author ewpatton@mit.edu (Evan W. Patton)
 */
Blockly.Versioning.findAllEventHandlers = function (
  dom,
  componentType,
  eventName
) {
  ...
  for (var i = 0; i < dom.children.length; i++) {
    ...
    if (
      block.tagName === "block" &&
      block.getAttribute("type") === "component_event"
    ) {
      var mutation = Blockly.Versioning.firstChildWithTagName(
        block,
        "mutation"
      ...
      if (!mutation) {
        throw (
          "Did not find expected mutation child in Blockly.Versioning.findAllEventHandlers " +
          "with componentType = " +
          componentType +
          " and eventName = " +
          eventName
 ...
 *
 */
Blockly.Versioning.findAllMethodCalls = function (
  dom,
  componentType,
  methodName
) {
  ...
  for (var b = 0, block; (block = allBlocks[b]); b++) {
    if (block.getAttribute("type") == "component_method") {
      ...
      if (!mutation) {
        throw (
          "Did not find expected mutation child in " +
          "Blockly.Versioning.findAllMethodCalls with componentType = " +
          componentType +
          "and methodName = " +
          methodName
        );
 ...
 *
 */
Blockly.Versioning.findAllPropertyBlocks = function (
  dom,
  componentType,
  propertyName
) {
  ...
  for (var b = 0, block; (block = allBlocks[b]); b++) {
    if (block.getAttribute("type") == "component_set_get") {
      ...
      if (!mutation) {
        throw (
          "Did not find expected mutation child in " +
          "Blockly.Versioning.findAllPropertyBlocks with componentType = " +
          componentType +
          "and propertyName = " +
          propertyName
        );
...
Blockly.Versioning.renameComponentType = function (
  componentType,
  newComponentType
) {
  return function (blocksRep) {
...
Blockly.Versioning.AllUpgradeMaps = {
  ...
  Ball: {
    ...
    */
    ...
    8: Blockly.Versioning.makeMethodUseDropdown(
      "Ball",
      "Bounce",
      0,
      "Direction"
  ...
  Canvas: {
    ...
    */
    ...
    8: [
      ...
      Blockly.Versioning.changeEventParameterName(
        "Canvas",
        "Dragged",
        "draggedSprite",
        "draggedAnySprite"
    ...
    9: Blockly.Versioning.addDefaultMethodArgument(
      "Canvas",
      "DrawCircle",
  ...
  ChartData2D: {}, // End ChartData2D upgraders
  ...
  DataFile: {}, // End DataFile upgraders
  ...
  EmailPicker: {
    ...
    */
    ...
    7: [
      Blockly.Versioning.changeMethodName(
        "EmailPicker",
        "SetCursorAt",
        "MoveCursorTo"
      ),
      ...
      Blockly.Versioning.changeEventName(
        "EmailPicker",
        "OnTextChanged",
        "TextChanged"
      ),
  ...
  Language: {
    ...
    */
    ...
    20: Blockly.Versioning.renameBlockType(
      "obsufcated_text",
      "obfuscated_text"
    ),
    ...
    36: [
      Blockly.Versioning.renameBlockType(
        "lists_minimum_number",
        "lists_minimum_value"
      ),
      Blockly.Versioning.renameBlockType(
        "lists_maximum_number",
        "lists_maximum_value"
      ),
    ],
  ...
  Map: {
    ...
    6: [
      ...
      Blockly.Versioning.makeSetterUseDropdown("Map", "MapType", "MapType"),
    ],
  }, // End Map upgraders

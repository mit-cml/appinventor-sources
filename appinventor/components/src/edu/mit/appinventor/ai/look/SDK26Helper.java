...
class SDK26Helper {
  ...
  static void askForPermission(final Look look, final Runnable next) {
    look.getForm().askPermission(Manifest.permission.CAMERA, new PermissionResultHandler() {
      @Override
      public void HandlePermissionResponse(String permission, boolean granted) {
        if (granted) {
          next.run();
        } else {
          look.getForm().PermissionDenied(look, "WebViewer", permission);
        }
      }
    });
  }

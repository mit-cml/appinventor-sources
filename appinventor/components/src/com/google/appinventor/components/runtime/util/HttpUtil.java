...
public class HttpUtil {
  ...
  private static SSLSocketFactory sslSockFactory = null; // Socket Factory for using
  ...
  private static synchronized void ensureSslSockFactory() {
   ...
   *
   */
  private static X509Certificate[] getSystemCertificates() {
    try {
  ...
  public static void post(String tag, String serviceUrl, byte[] content, HttpCallback callback) {
    ...
    try {
      ...
      if (connection != null) {
        try {
          ...
          byte[] body = IOUtils.readStream(connection.getInputStream());
          if (callback != null) {
            callback.onResponse(responseCode, body, null);
          }
        } finally {
      ...
      callback.onResponse(-1, null, e.toString());
    }
  }
}

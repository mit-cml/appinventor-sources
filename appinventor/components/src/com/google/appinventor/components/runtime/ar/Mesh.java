...
public class Mesh implements Closeable {
    ...
    public static Mesh createFromAsset(ARViewRender render, String assetFileName) throws IOException {
        try (InputStream inputStream = render.getForm().openAsset(assetFileName)) {
            ...
            Log.i(TAG, "Loading model with " + obj.getNumVertices() + " vertices and " +
                    vertexIndices.capacity() + " indices");
            ...
            // Add this to your createFromAsset method

            int numVertices = obj.getNumVertices();
            int numFaces = obj.getNumFaces();
            Log.i(TAG, "Model statistics - Vertices: " + numVertices +
                    ", Faces: " + numFaces +
                    ", Estimated memory: " +
                    ((numVertices * (3 + 2 + 3) * 4) + (numFaces * 3 * 4)) + " bytes");


            // Add this to your createFromAsset method
            Log.i(TAG, "Buffer sizes - Vertices: " + localCoordinates.capacity() * 4 + " bytes, " +
                    "TexCoords: " + textureCoordinates.capacity() * 4 + " bytes, " +
                    "Normals: " + normals.capacity() * 4 + " bytes, " +
                    "Indices: " + vertexIndices.capacity() * 4 + " bytes");

            IndexBuffer indexBuffer = new IndexBuffer(render, vertexIndices);

            Log.i(TAG, "returning mesh with indices");

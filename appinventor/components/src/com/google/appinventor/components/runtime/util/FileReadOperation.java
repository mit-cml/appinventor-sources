 ...
 */
public class FileReadOperation extends FileOperation<InputStream> {
  ...
  public FileReadOperation(Form form, Component component, String method, String fileName,
      FilePermissionMode permissionMode, boolean async) {
    super(form, component, method, fileName, permissionMode, FileAccessMode.READ, async);
  }

  @Override
  protected boolean process(InputStream stream) throws IOException {
    return process(IOUtils.readStream(openFile()));
  }
  ...
  InputStream openFile() throws IOException {
    if (FileUtil.isAssetUri(form, resolvedPath)) {
      return form.openAsset(fileName.substring(2));
    }
    return FileUtil.openFile(form, URI.create(resolvedPath));
  }
}

...
import com.google.appinventor.components.runtime.translate.TranslateToken;
 ...
 */
@DesignerComponent(version = YaVersion.TRANSLATE_COMPONENT_VERSION,
    ...
    nonVisible = true,
    iconName = "images/translate.png")
@UsesPermissions(permissionNames = "android.permission.INTERNET")
@UsesLibraries(libraries = "protobuf-java-3.0.0.jar")
@SimpleObject
public final class Translate extends AndroidNonvisibleComponent {
  ...
  public static final String TRANSLATE_SERVICE_URL =
   ...
   */
  public Translate(ComponentContainer container) {
   ...
   *
   *   **Note:** Translate will attempt to detect the source language. You can also specify
  ...
  private void performRequest(String languageToTranslateTo, String textToTranslate) {
    ...
    try {
      ...
      TranslateToken.token token = TranslateToken.token.parseFrom(decodedToken);
      TranslateToken.request request = TranslateToken.request.newBuilder()
        .setToken(token)
        .setTotranslate(textToTranslate)
        .setSourcelanguage(srcLanguage)
        .setTargetlanguage(targetLanguage).build();

      URL url = new URL(TRANSLATE_SERVICE_URL);
      ...
      if (connection != null) {
        try {
          ...
          TranslateToken.response response = TranslateToken.response.parseFrom(connection.getInputStream());
          String returnText;
          if (responseCode == 200) {
  ...
  public void GotTranslation(final String responseCode, final String translation) {
    activity.runOnUiThread(new Runnable() {
        @Override
        public void run() {
          EventDispatcher.dispatchEvent(Translate.this, "GotTranslation", responseCode, translation);
        }
      });
  }
  ...
  private synchronized void ensureSslSockFactory() {
    if (SslSockFactory != null) {
      ...
      try {
        ...
        throw new YailRuntimeError("Could Not setup SSL Trust Store for Translate: ", e.getMessage());
      }
    }
  }

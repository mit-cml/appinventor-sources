...
public class SpecularCubeMapFilter implements Closeable {
    ...
    private static final FloatBuffer COORDS_BUFFER = ByteBuffer.allocateDirect(COORDS_BUFFER_SIZE).order(ByteOrder.nativeOrder()).asFloatBuffer();

    static {
        COORDS_BUFFER.put(new float[]{
                /*0:*/ -1f, -1f, /*1:*/ +1f, -1f, /*2:*/ -1f, +1f, /*3:*/ +1f, +1f,});
    }

    private static final String[] ATTACHMENT_LOCATION_DEFINES = {"PX_LOCATION", "NX_LOCATION", "PY_LOCATION", "NY_LOCATION", "PZ_LOCATION", "NZ_LOCATION",};

    private static final int[] ATTACHMENT_ENUMS = {GLES30.GL_COLOR_ATTACHMENT0, GLES30.GL_COLOR_ATTACHMENT1, GLES30.GL_COLOR_ATTACHMENT2, GLES30.GL_COLOR_ATTACHMENT3, GLES30.GL_COLOR_ATTACHMENT4, GLES30.GL_COLOR_ATTACHMENT5,};
     ...
     */
    public SpecularCubeMapFilter(ARViewRender render, int resolution, int numberOfImportanceSamples) throws IOException {
        this.resolution = resolution;
        this.numberOfImportanceSamples = numberOfImportanceSamples;
        this.numberOfMipmapLevels = log2(resolution) + 1;

        try {
            radianceCubemap = new Texture(render, Texture.Target.TEXTURE_CUBE_MAP, Texture.WrapMode.CLAMP_TO_EDGE);
            ldCubemap = new Texture(render, Texture.Target.TEXTURE_CUBE_MAP, Texture.WrapMode.CLAMP_TO_EDGE);
            ...
            mesh = new Mesh(render, Mesh.PrimitiveMode.TRIANGLE_STRIP,
                    /*indexBuffer=*/ null, new VertexBuffer[]{coordsBuffer});
        } catch (Throwable t) {
            close();
            throw t;
        }
    ...
    public void close() {
        if (framebuffers != null) {
            for (int[] framebufferChunks : framebuffers) {
                ...
                GLError.maybeLogGLError(Log.WARN, TAG, "Failed to free framebuffers", "glDeleteFramebuffers");
            }
        }
        ...
        try {
            ...
            for (int i = 0; i < NUMBER_OF_CUBE_FACES; ++i) {
                ...
                if (image.getHeight() != resolution) {
                    throw new IllegalArgumentException("Cubemap face resolution (" + image.getHeight() + ") does not match expected value (" + resolution + ").");
                }

                GLES30.glTexImage2D(GLES30.GL_TEXTURE_CUBE_MAP_POSITIVE_X + i,
                        /*level=*/ 0, GLES30.GL_RGBA16F,
                        /*width=*/ resolution,
                        /*height=*/ resolution,
                        /*border=*/ 0, GLES30.GL_RGBA, GLES30.GL_HALF_FLOAT, image.getPlanes()[0].getBuffer());
    ...
    private void initializeLdCubemap() {
        ...
        for (int level = 0; level < numberOfMipmapLevels; ++level) {
            ...
            for (int face = 0; face < NUMBER_OF_CUBE_FACES; ++face) {
                GLES30.glTexImage2D(GLES30.GL_TEXTURE_CUBE_MAP_POSITIVE_X + face, level, GLES30.GL_RGB16F,
                        /*width=*/ mipmapResolution,
                        /*height=*/ mipmapResolution,
                        /*border=*/ 0, GLES30.GL_RGB, GLES30.GL_HALF_FLOAT,
                        /*data=*/ null);
    ...
    private Shader[] createShaders(ARViewRender render, ChunkIterable chunks) throws IOException {
        ...
        for (Chunk chunk : chunks) {
            ...
            shaders[chunk.chunkIndex] = Shader.createFromAssets(render, "cubemap_filter.vert", "cubemap_filter.frag", defines).setTexture("u_Cubemap", radianceCubemap).setDepthTest(false).setDepthWrite(false);
        }
        ...
        for (Shader shader : shaders) {
            for (int i = 0; i < importanceSampleCaches.length; ++i) {
                ...
                for (int j = 0; j < cache.length; ++j) {
                    ...
                    shader.setVec3(entryName + ".direction", entry.direction).setFloat(entryName + ".contribution", entry.contribution).setFloat(entryName + ".level", entry.level);
                }
            }
        }
        ...
        for (int level = 0; level < numberOfMipmapLevels; ++level) {
            ...
            for (Chunk chunk : chunks) {
                ...
                for (int attachment = 0; attachment < chunk.chunkSize; ++attachment) {
                    GLES30.glFramebufferTexture2D(GLES30.GL_FRAMEBUFFER, GLES30.GL_COLOR_ATTACHMENT0 + attachment, GLES30.GL_TEXTURE_CUBE_MAP_POSITIVE_X + chunk.firstFaceIndex + attachment, ldCubemap.getTextureId(), level);

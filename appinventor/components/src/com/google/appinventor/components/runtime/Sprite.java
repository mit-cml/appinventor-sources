 ...
 *
 * While the Simple programmer sees the x- and y-coordinates as integers, they
 * are maintained internally as doubles so fractional changes (caused by
 * multiplying the speed by a cosine or sine value) have the chance to add up.
 *
...
public abstract class Sprite extends VisibleComponent implements AlarmHandler, OnDestroyListener, Deleteable {
	...
	private static final boolean DEFAULT_ENABLED = true; // Enable timer for
															// movement
	...
	// That way, we don't raise CollidedWith() more than once for each
	// collision.
	...
	/**
	 * The angle, in degrees <em>below</em> the positive x-axis, specified by
	 * the user. We use this to compute new coordinates because, on Android, the
	...
	/**
	 * Creates a new Sprite component. This version exists to allow injection of
	 * a mock handler for testing.
	 *
	 * @param container
	 *            where the component will be placed
	 * @param handler
	 *            a scheduler to which runnable events will be posted
	 */
	 ...
	 * local variables {@link #userHeading} and {@link #heading}, this sets
	 * {@link #headingCos}, {@link #headingSin}, and {@link #headingRadians}.
	 *
	 * @param userHeading
	 *            degrees above the positive x-axis
	 */
	@SimpleProperty(category = PropertyCategory.BEHAVIOR)
	@DesignerProperty(editorType = PropertyTypeConstants.PROPERTY_TYPE_FLOAT, defaultValue = DEFAULT_HEADING + "")
	public void Heading(double userHeading) {
		this.userHeading = userHeading;
		// Flip, because y increases in the downward direction on Android
		// canvases
		heading = -userHeading;
		headingRadians = Math.toRadians(heading);
		headingCos = Math.cos(headingRadians);
		headingSin = Math.sin(headingRadians);
		// changing the heading needs to force a redraw for image sprites that
		// rotate
		registerChange();
	}
	 ...
	 */
	@SimpleProperty(description = "Returns the sprite's heading in degrees above the positive "
	 ...
	 */
	@SimpleProperty(description = "The interval in milliseconds at which the sprite's "
			+ "position is updated.  For example, if the interval is 50 and the speed is 10, "
			+ "then the sprite will move 10 pixels every 50 milliseconds.", category = PropertyCategory.BEHAVIOR)
	public int Interval() {
	 ...
	 */
	@DesignerProperty(editorType = PropertyTypeConstants.PROPERTY_TYPE_NON_NEGATIVE_INTEGER, defaultValue = DEFAULT_INTERVAL
			+ "")
	@SimpleProperty
	 ...
	 *
	 * @param speed
	 *            the magnitude (in pixels) to move every {@link #interval}
	 *            milliseconds
	 */
	@SimpleProperty(category = PropertyCategory.BEHAVIOR)
	@DesignerProperty(editorType = PropertyTypeConstants.PROPERTY_TYPE_FLOAT, defaultValue = DEFAULT_SPEED + "")
	 ...
	 *
	 * @return the magnitude (in pixels) the sprite moves every
	 *         {@link #interval} milliseconds.
	 */
	@SimpleProperty(description = "he speed at which the sprite moves.  The sprite moves "
	 ...
	 *
	 * @param visible
	 *            {@code true} if the sprite should be visible; {@code false}
	 *            otherwise.
	 */
	@DesignerProperty(editorType = PropertyTypeConstants.PROPERTY_TYPE_BOOLEAN, defaultValue = DEFAULT_VISIBLE ? "True"
			: "False")
	@SimpleProperty
	...
	@SimpleProperty(description = "The horizontal coordinate of the left edge of the sprite, "
	...
	@SimpleProperty(description = "The vertical coordinate of the top of the sprite, "
	...
	/**
	 * Sets the layer of the sprite, indicating whether it will appear in front
	 * of or behind other sprites.
	 *
	 * @param layer
	 *            higher numbers indicate that this sprite should appear in
	 *            front of ones with lower numbers; if values are equal for
	 *            sprites, either can go in front of the other
	 */
	@SimpleProperty(category = PropertyCategory.APPEARANCE)
	@DesignerProperty(editorType = PropertyTypeConstants.PROPERTY_TYPE_FLOAT, defaultValue = DEFAULT_Z + "")
	public void Z(double layer) {
		this.zLayer = layer;
		canvas.changeSpriteLayer(this); // Tell canvas about change
	}

	@SimpleProperty(description = "How the sprite should be layered relative to other sprits, "
	...
	// Simple events: CollidedWith, Dragged, EdgeReached, Touched,
	// NoLongeCollidingWith,
	...
	/**
	 * Handler for CollidedWith events, called when two sprites collide. Note
	 * that checking for collisions with a rotated ImageSprite currently checks
	 * against the sprite's unrotated position. Therefore, collision checking
	 * will be inaccurate for tall narrow or short wide sprites that are
	 * rotated.
	 *
	 * @param other
	 *            the other sprite in the collision
	 */
	@SimpleEvent
	...
	/**
	 * Handler for Dragged events. On all calls, the starting coordinates are
	 * where the screen was first touched, and the "current" coordinates
	 ...
	 * from the prior call. Note that the Sprite won't actually move anywhere in
	...
	/**
	 * Event handler called when the sprite reaches an edge of the screen. If
	 * Bounce is then called with that edge, the sprite will appear to bounce
	 * off of the edge it reached.
	 */
	@SimpleEvent(description = "Event handler called when the sprite reaches an edge of the screen. "
	...
	public void EdgeReached(int edge) {
		if (edge == Component.DIRECTION_NONE || edge < Component.DIRECTION_MIN || edge > Component.DIRECTION_MAX) {
	 ...
	 *
	 * @param other
	 *            the sprite formerly colliding with this sprite
	 */
	...
	/**
	 * When a fling gesture (quick swipe) is made on the sprite: provides the
	 * (x,y) position of the start of the fling, relative to the upper left of
	 * the canvas. Also provides the speed (pixels per millisecond) and heading
	 ...
	 *            x-coordinate of touched point
	 * @param y
	 *            y-coordinate of touched point * @param speed the speed of the
	 *            fling sqrt(xspeed^2 + yspeed^2)
	 * @param heading
	 *            the heading of the fling
	 * @param xvel
	 *            the speed in x-direction of the fling
	 * @param yvel
	 *            the speed in y-direction of the fling
	 * 
	 */
	@SimpleEvent
	...
	/**
	 * When the user stops touching the sprite (lifts finger after a TouchDown
	 * event): provides the (x,y) position of the touch, relative to the upper
	 * left of the canvas
	 *
	 * @param x
	 *            x-coordinate of touched point
	 ...
	 * leaves it there): provides the (x,y) position of the touch, relative to
	 ...
	 * direction). This also calls {@link #MoveIntoBounds()} in case the sprite
	 * is out of bounds.
	 *
	 * @param edge
	 ...
	 *            ,
	 *            {@link com.google.appinventor.components.runtime.Component#DIRECTION_WEST}
	 *            , or
	...
	public void Bounce(int edge) {
		...
		// The following step is necessary because Java's modulus operation
		// yields a
		// negative number if the dividend is negative and the divisor is
		// positive.
		...
		if ((edge == Component.DIRECTION_EAST && (normalizedAngle < 90 || normalizedAngle > 270))
				|| (edge == Component.DIRECTION_WEST && (normalizedAngle > 90 && normalizedAngle < 270))) {
			Heading(180 - normalizedAngle);
		} else if ((edge == Component.DIRECTION_NORTH && normalizedAngle > 0 && normalizedAngle < 180)
				|| (edge == Component.DIRECTION_SOUTH && normalizedAngle > 180)) {
			Heading(360 - normalizedAngle);
		} else if ((edge == Component.DIRECTION_NORTHEAST && normalizedAngle > 0 && normalizedAngle < 90)
				|| (edge == Component.DIRECTION_NORTHWEST && normalizedAngle > 90 && normalizedAngle < 180)
				|| (edge == Component.DIRECTION_SOUTHWEST && normalizedAngle > 180 && normalizedAngle < 270)
	...
	/**
	 * Indicates whether a collision has been registered between this sprite and
	 * the passed sprite.
	 *
	 * @param other
	 *            the sprite to check for collision with this sprite
	 ...
	 *
	 * @param target
	 *            the other sprite to point towards
	 */
	@SimpleFunction(description = "Turns the sprite to point towards a designated "
	 ...
	 *
	 * @param x
	 *            parameter of the point to turn to
	 * @param y
	 *            parameter of the point to turn to
	 */
	...
	public void PointInDirection(double x, double y) {
		Heading(-Math.toDegrees(Math.atan2(
				...
				y - Y() - Height() / 2, x - X() - Width() / 2)));
	}

	// Internal methods supporting move-related functionality

	/**
	 * Responds to a move or change of this sprite by redrawing the enclosing
	 * Canvas and checking for any consequences that need handling.
	 * Specifically, this (1) notifies the Canvas of a change so it can detect
	 * any collisions, etc., and (2) raises the {@link #EdgeReached(int)} event
	 * if the Sprite has reached the edge of the Canvas.
	 */
	protected void registerChange() {
		// This was added to fix bug 2262218, where Ball.CollidedWith() was
		// called
		// before all components had been constructed.
		if (!initialized) {
			// During REPL, components are not initalized, but we still want to
			// repaint the canvas.
	...
	/**
	 * Specifies which edge of the canvas has been hit by the Sprite, if any,
	 * moving the sprite back in bounds.
	 *
	...
	protected int hitEdge(int canvasWidth, int canvasHeight) {
		...
		// the sprite is past the northwest boundary, north and west will be
		// true.
		...
		// last move, since that might have been multiple pixels, and we'd only
		// need
		// to undo part of it.
		MoveIntoBounds();
	...
	public BoundingBox getBoundingBox(int border) {
		return new BoundingBox(X() - border, Y() - border, X() + Width() - 1 + border, Y() + Height() - 1 + border);
	}
	 ...
	 * Subclasses of Sprite that are not rectangular should override this
	 * method.
	 *
	 * @param qx
	 *            the x-coordinate
	...
	public boolean containsPoint(double qx, double qy) {
		return qx >= xLeft && qx < xLeft + Width() && qy >= yTop && qy < yTop + Height();
	}

	// Convenience methods for dealing with hitting the screen edge and
	// collisions
	 ...
	 */
	public void alarm() {
		// This check on initialized is currently redundant, since
		// registerChange()
		// checks it too.

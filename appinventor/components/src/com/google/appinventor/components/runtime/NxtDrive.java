 ...
 */
@DesignerComponent(version = YaVersion.NXT_DRIVE_COMPONENT_VERSION, description = "A component that provides a high-level interface to a LEGO MINDSTORMS NXT "
		+ "robot, with functions that can move and turn the robot.", category = ComponentCategory.LEGOMINDSTORMS, nonVisible = true, iconName = "images/legoMindstormsNxt.png")
@SimpleObject
public class NxtDrive extends LegoMindstormsNxtBase {
	...
	public NxtDrive(ComponentContainer container) {
		...
		DriveMotors("CB"); // C & B are the left & right drive motors of the
							// ShooterBot robot.
		WheelDiameter(4.32f);
	...
	public void beforeDisconnect(BluetoothConnectionBase bluetoothConnection) {
		if (stopBeforeDisconnect) {
			for (int port : driveMotorPorts) {
				setOutputState("Disconnect", port, 0, MODE_BRAKE,
						REGULATION_MODE_IDLE, 0, MOTOR_RUN_STATE_IDLE, 0);
			}
		}
	}
	...
	@SimpleProperty(description = "The motor ports that are used for driving: the left wheel's "
			+ "motor port followed by the right wheel's motor port.", category = PropertyCategory.BEHAVIOR, userVisible = false)
	 ...
	 */
	@DesignerProperty(editorType = PropertyTypeConstants.PROPERTY_TYPE_STRING, defaultValue = "CB")
	@SimpleProperty
	 ...
	 */
	@DesignerProperty(editorType = PropertyTypeConstants.PROPERTY_TYPE_FLOAT, defaultValue = "4.32")
	@SimpleProperty
	...
	public void MoveForward(int power, double distance) {
		...
		// This doesn't work accurately, but it is the best we can do with
		// bluetooth direct commands.
	...
	private void move(String functionName, int power, long tachoLimit) {
		...
		for (int port : driveMotorPorts) {
			setOutputState(functionName, port, power, MODE_MOTORON,
					REGULATION_MODE_MOTOR_SPEED, 0, MOTOR_RUN_STATE_RUNNING,
					tachoLimit);
		}
	}
	...
	public void TurnClockwiseIndefinitely(int power) {
		...
		if (numDriveMotors >= 2) {
			...
			turnIndefinitely("TurnClockwiseIndefinitely", power,
					forwardMotorIndex, backwardMotorIndex);
		}
	}
	...
	public void TurnCounterClockwiseIndefinitely(int power) {
		...
		if (numDriveMotors >= 2) {
			...
			turnIndefinitely("TurnCounterClockwiseIndefinitely", power,
					forwardMotorIndex, backwardMotorIndex);
		}
	}

	private void turnIndefinitely(String functionName, int power,
			int forwardMotorIndex, int reverseMotorIndex) {
		if (!checkBluetooth(functionName)) {
			return;
		}

		setOutputState(functionName, driveMotorPorts.get(forwardMotorIndex),
				power, MODE_MOTORON, REGULATION_MODE_MOTOR_SPEED, 0,
				MOTOR_RUN_STATE_RUNNING, 0);
		setOutputState(functionName, driveMotorPorts.get(reverseMotorIndex),
				-power, MODE_MOTORON, REGULATION_MODE_MOTOR_SPEED, 0,
				MOTOR_RUN_STATE_RUNNING, 0);
	}

	// TODO(lizlooney) - it would be nice to have TurnClockwise and
	// TurnCounterClockwise (or
	// TurnRight and TurnLeft?) methods that take an angle (in degrees). I think
	// we'd need to know
	// the distance between the drive wheels, so that could be a property
	// (similar to
	...
	public void Stop() {
		...
		for (int port : driveMotorPorts) {
			setOutputState(functionName, port, 0, MODE_BRAKE,
					REGULATION_MODE_IDLE, 0, MOTOR_RUN_STATE_IDLE, 0);
		}
	}
}

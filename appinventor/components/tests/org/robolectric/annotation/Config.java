/*
 * NOTE: This is only here to work around a bug in JDK < 1.7u80 where javac cannot handle
 * annotations that have a <clinit> entry. Once we allow for newer versions of the JDK this can
 * be removed and the original copy of Config can be returned to the robolectric JAR.
 */
...
public @interface Config {
  ...
  class Implementation implements Config {
    ...
    private static int parseSdkInt(String part) {
      ...
      if (spec.equals("ALL_SDKS")) {
        return Config.ALL_SDKS;
      } else if (spec.equals("TARGET_SDK")) {
        return Config.TARGET_SDK;
      } else if (spec.equals("OLDEST_SDK")) {
        return Config.OLDEST_SDK;
      } else if (spec.equals("NEWEST_SDK")) {
  ...
  class Builder {
    ...
    public Builder overlay(Config overlayConfig) {
      ...
      Set<String> instrumentedPackages = new HashSet<String>();

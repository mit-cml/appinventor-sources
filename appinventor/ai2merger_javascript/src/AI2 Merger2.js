...
function createNewFilePath(path, newName) {
	return path.substring(0, path.lastIndexOf('/') + 1) + newName + path.substring(path.lastIndexOf('.'));
}
			  ...
			  try {
				...
				$.each(zip.files, function (index, zipEntry) {
				  ...
				  if (!(getFileName(zipEntry.name) == "Screen1") || num < 1) {
					...
					!(getExt(zipEntry.name) == ".yail") || !(getExt(zipEntry.name) == ".properties")) {
					  // For media files.
					  ...
					  if (screenNum == 0) {
					    newName = prompt("Your project can only have one Screen1. Rename Screen1 from " + fileName + " to continue.");
					  }
					  var text = zipEntry.asText();

					  if (getExt(zipEntry.name) == ".scm") {
						text = zipEntry.asText().replace("\"$Name\":\"Screen1\"", "\"$Name\":\"" + newName + "\"");

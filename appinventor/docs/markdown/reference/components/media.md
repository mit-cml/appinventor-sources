 ...
 **Note:** Translation happens asynchronously in the background. When the translation is complete,
 the [`GotTranslation`](#Translate.GotTranslation) event is triggered.
...
{:id="Translate.ApiKey" .text .wo} *ApiKey*
...
{:id="Translate.GotTranslation"} GotTranslation(*responseCode*{:.text},*translation*{:.text})
...
{:id="Translate.RequestTranslation" class="method"} <i/> RequestTranslation(*languageToTranslateTo*{:.text},*textToTranslate*{:.text})
: By providing a target language to translate to (for instance, 'es' for Spanish, 'en' for
 ...
 service, the event [`GotTranslation`](#Translate.GotTranslation) will be executed.
